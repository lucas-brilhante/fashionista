...
const ProductInfo = () => {
  ...
  const addItem = (item_, size) => () => {
    dispatch(addItemToCard(item_, size))
  ...
  useEffect(() => {
    if (item) {
      item.sizes.some((size) => {
        if (size.available) setSizeSelected(size.size)
    ...
    return (
      <ProductContainer>
        ...
        <ProductInfoContainer>
            ...
            {sizes.map((size) => {
              if (size.available)
                return (
                  <ProductSize
                    key={size.sku}
                    actived={sizeSelected === size.size}
          ...
          <AddProductToCard onClick={addItem(item, sizeSelected)}>
            Adicionar à Sacola
          </AddProductToCard>
        </ProductInfoContainer>
      </ProductContainer>
...
const ProductSize = styled.button`
  ...
  border: ${({ actived }) =>
    actived ? '2px solid black' : '1px solid rgba(0,0,0,0.2)'};
